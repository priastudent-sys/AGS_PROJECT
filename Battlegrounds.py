...
while player['hp'] > 0 and enemy['hp'] > 0 :
    print(f"--=-- HP of {player['nama']} :: {player['hp']} || HP of {enemy['nama']} :: {enemy['hp']} --=--")
    ...
    pilihan = input("Mau pakai nomor 1 atau 2? ")
    ...
    print(f"the {player['nama']} is reduced by <<< {e_dmg} >>>")

    time.sleep(1.5)

    if player['hp'] == enemy['hp']:
        print(f"/n its a tie, both player and enemy have the same hp {player['hp']} /n")
        time.sleep(1.5)
    
if player['hp'] < enemy['hp']:
    print(f"your hp is lower than your enemy, you need to be more careful")
    time.sleep(1.5)
    ...
    print("you are the champion of battlegrounds\n\n")